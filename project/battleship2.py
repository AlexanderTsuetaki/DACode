# battleship_game.py
# Class-based version of battleship game, found here:
# http://thelivingpearl.com/2014/02/17/the-game-of-battleships-in-python/
# The AI for this game is AWFUL -- your job is to improve it.
# Due date: Sunday, March 6
__author__ = "Alexnder Tsuetaki"
...
def computer_move(board):
    ...
    if hitCount==1
        ...
        left = cords1[2]-1
        right = cords1[2]+1
        if grid[up,cords1[2]]!="*" and grid[up,cords1[2]] != "$"
            testcords.append[up,cords1[2]]
        if grid[down,cords1[2]]!="*" and grid[down,cords1[2]] != "$"
            testcords.append[down,cords1[2]]
        if grid[cords1[1],left]!="*" and grid[cords1[1],left] != "$"
            testcords.append[cords[1],left]
        if grid[cords1[1],right]!="*" and grid[cords1[1],right] != "$"
            testcords.append[cords1[1],right]
        randomnNewChoice = random.choice(testcords)
        x=randomNewChoice[1]
        y=randomNewChoice[2]
        res = make_move(board,x ,y )
        if res == "hit":
            print "Hit at " + str(x + 1) + "," + str(y + 1)
            check_sink(board, x, y)
            if cords2[1]-cords1[1]==1:
                Right= True
            elif cords1[1]-cords2[1]==1:
                Right = False
            elif cords1[2]-cords2[2]==1:
                Up=False
            else:
                Up = True
            board[x][y] = '$'
